...
for i in np.arange(heights.shape[0]):
    plt.plot(closedResponse1[i][:,0],closedResponse1[i][:,1], linewidth=2, label = str(heights[i]*1e3) + ' mN-m')
...
ax1.yaxis.set_ticks_position('left')
for i in np.arange(heights.shape[0]):
    plt.plot(closedResponse2[i][:,0],closedResponse2[i][:,1], linewidth=2, label = str(heights[i]*1e3) + ' mN-m')
